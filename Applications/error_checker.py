...
class ErrorChecker:
    ...
    def check_error( meta_data_fragment, user_input ):
        ...
        if type(data_type) == cx_Oracle.NUMBER:
            ...
            precision = meta_data_fragment[4]
            ...
            internal_size = meta_data_fragment[3]
            if ( 10**internal_size < user_input ):
                return False

        # Check string objects
        elif type(data_type) == cx_Oracle.STRING:
            desired_length = metad_data_fragment[3]
            if ( len( user_input ) > desired_length ):
                return False
            
        # Check datetime objects
        elif type(data_type) == cx_Oracle.DATETIME:
            try:
                dd = user_input[0]
                mm = user_input[1]
                yyyy = user_input[2]
